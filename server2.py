...
def _payload_foto(fila):
    ...
    if contenido:
        ...
        # El navegador no abre el bucket de Cloudflare; las miniaturas van por la API.
        data = url_api
...
def crear_faena():
    ...
    cliente = conn.execute(
        "SELECT nombre, intermediario_id, direccion FROM clientes WHERE id=?",
        (cliente_id,),
    ).fetchone()
    if not cliente:
        conn.close()
    ...
    carpeta = crear_carpeta_faena(numero, cliente["nombre"])
    direccion = (datos.get("direccion") or "").strip() or (cliente["direccion"] or "").strip()
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO faenas
